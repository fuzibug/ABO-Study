...
const questionGenerators = {
    general: [
        ...
        () => ({
            q: "What is the typical add power for a 65-year-old presbyopic patient?",
            opts: ["+1.00D", "+1.50D", "+2.00D", "+2.50D"],
            correct: 3,
            exp: "By age 65, most patients require +2.50D add power for near vision as accommodation is fully lost."
        }),
        () => ({
    ...
    optics: [
        ...
        () => {
            ...
            const newAxis = axis === 90 ? 180 : axis === 180 ? 90 : axis;
    ...
    dispensing: [
        () => {
            const pd = randomChoice([60, 61, 62, 63, 64]);
            return {
        ...
        () => {
            const tilt = randomChoice([8, 9, 10, 11, 12]);
            return {
    ...
    products: [
        ...
        () => {
            const reduction = randomChoice([8, 10, 12]);
            return {
        ...
        () => {
            const thickness = randomChoice([1.5, 2.0, 2.2]);
            return {
        ...
        () => {
            const reduction = randomChoice([25, 30, 35]);
            return {
...
function generateUniqueQuestions(domain, count) {
    const generators = domain === 'all' 
        ? [...questionGenerators.general, ...questionGenerators.optics, 
           ...questionGenerators.dispensing, ...questionGenerators.products]
        : questionGenerators[domain];
    ...
    // If we need more questions than unique generators, start regenerating with new random values
    while (questions.length < count) {
        const idx = Math.floor(Math.random() * generators.length);
        questions.push(generators[idx]()); // Generates new random values
    }
    
    return questions;
}

// Build static question bank (for compatibility with existing code)
const questionBank = {
    general: Array.from({ length: 20 }, (_, i) => 
        questionGenerators.general[i % questionGenerators.general.length]()
    ),
    optics: Array.from({ length: 20 }, (_, i) => 
        questionGenerators.optics[i % questionGenerators.optics.length]()
    ),
    dispensing: Array.from({ length: 20 }, (_, i) => 
...
if (typeof module !== 'undefined' && module.exports) {
    module.exports = { questionBank, questionGenerators, generateUniqueQuestions };
}
